use crate::types_2_0::*;
use crate::parser_2_0::{Reader, Optionals};
...
impl<'a> Reader<'a> {
    ...
    pub fn read_child_info(&mut self, len: u32) -> ChildInfoSlice<'a> {
        ChildInfoSlice {
            len,
            hash: HashRangeSlice(self.read_range(len)),
            ...
            sub_meshes: SubMeshProjectionRangeSlice(self.read_range(len)),
        }
    }
    ...
    pub fn read_sub_mesh(&mut self, len: u32) -> SubMeshSlice<'a> {
        SubMeshSlice {
            ...
            attributes: self.read_slice(len),
            num_deviations: self.read_slice(len),
            vertices: VertexRangeSlice(self.read_range(len)),
            primitive_vertex_indices: VertexIndexRangeSlice(self.read_range(len)),
            edge_vertex_indices: VertexIndexRangeSlice(self.read_range(len)),
            corner_vertex_indices: VertexIndexRangeSlice(self.read_range(len)),
            textures: TextureInfoRangeSlice(self.read_range(len)),
        }
    }
    ...
    pub fn read_texture_info(&mut self, len: u32) -> TextureInfoSlice<'a>  {
        TextureInfoSlice {
            ...
            pixel_range: PixelRangeSlice(self.read_range(len)),
        }
    }

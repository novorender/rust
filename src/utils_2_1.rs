...
impl<'a> Iterator for ChildInfoIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let descendant_object_ids = unsafe{ self.schema.descendant_object_ids.slice_range(&descendant_obj_ids_range) };
...
impl<'a> Schema<'a> {
    pub fn children(&self, separate_positions_buffer: bool, filter: impl Fn(u32) -> bool + Copy) -> impl ExactSizeIterator<Item = Child> {
